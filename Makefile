# This Makefile is for the Dancer::Plugin::Auth::Github extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Dancer/Plugin/Auth/Github.pm]
...
#     NAME => q[Dancer::Plugin::Auth::Github]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0], LWP::UserAgent=>q[6.04], JSON=>q[2.53], Dancer=>q[1.311] }
#     VERSION_FROM => q[lib/Dancer/Plugin/Auth/Github.pm]
#     clean => { FILES=>q[Dancer-Plugin-Auth-Github-*] }
...
NAME_SYM = Dancer_Plugin_Auth_Github
VERSION = 0.01
...
MAKE = make
FULLEXT = Dancer/Plugin/Auth/Github
BASEEXT = Github
PARENT_NAME = Dancer::Plugin::Auth
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Dancer/Plugin/Auth/Github.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Dancer/Plugin/Auth
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Dancer/Plugin/Auth
...
TO_INST_PM = lib/Dancer/Plugin/Auth/Github.pm

PM_TO_BLIB = lib/Dancer/Plugin/Auth/Github.pm \
...
DISTNAME = Dancer-Plugin-Auth-Github
DISTVNAME = Dancer-Plugin-Auth-Github-0.01
...
manifypods : pure_all  \
	lib/Dancer/Plugin/Auth/Github.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Dancer/Plugin/Auth/Github.pm $(INST_MAN3DIR)/Dancer::Plugin::Auth::Github.$(MAN3EXT) 
...
clean :: clean_subdirs
	- $(RM_F) \
	  ...
	  *$(OBJ_EXT) pm_to_blib \
	  ...
	  $(BASEEXT).exp 
	- $(RM_RF) \
	  Dancer-Plugin-Auth-Github-* blib 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Authenticate with Github'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Gideon Israel Dsouza <gideon@cpan.org>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Dancer-Plugin-Auth-Github' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '  LWP::UserAgent: 6.04' >> META_new.yml
	$(NOECHO) $(ECHO) '  perl: 5.006' >> META_new.yml
	$(NOECHO) $(ECHO) 'version: 0.01' >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) '{' > META_new.json
	$(NOECHO) $(ECHO) '   "abstract" : "Authenticate with Github",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Gideon Israel Dsouza <gideon@cpan.org>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Dancer-Plugin-Auth-Github",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '            "LWP::UserAgent" : "6.04",' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Authenticate with Github</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Gideon Israel Dsouza &lt;gideon@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Dancer/Plugin/Auth/Github.pm blib/lib/Dancer/Plugin/Auth/Github.pm 
